...
<div class="modal fade" id="editLessonModal" tabindex="-1" aria-labelledby="editUserLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content shadow">
      <div class="modal-header">
        <h5 class="modal-title" id="editUserLabel">Ders Bilgilerini Düzenle</h5>
      ...
      <form method="POST">
        <div class="modal-body">
          ...
          <div class="mb-3">
            <label for="editLesson" class="form-label">Öğretmen</label>
            <select class="form-select" name="teacher" id="edit-teacher" required>
              <?php foreach($teachers as $teacher): ?>
                <option value="<?php echo htmlspecialchars($teacher['id']) ?>"><?php echo htmlspecialchars($teacher['username'])?></option>
              <?php endforeach;?>
            </select>
          </div>

          <div class="mb-3">
            <label for="editLesson" class="form-label">Kategoriler</label>
...
document.addEventListener('DOMContentLoaded', function () {
  const editButtons = document.querySelectorAll('[data-bs-target="#editLessonModal"]');
  ...
  editButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      const categories = button.getAttribute('data-category');
      const teacher = button.getAttribute('teacher_id');
