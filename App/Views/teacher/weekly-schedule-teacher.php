...
<script>
    ...
    const apiUrl = `/akademikx/public/get-schedules-teachers/${userId}`;
    ...
    // Saatleri oluştur (05:00 - 22:00 arası)
    const timeSlots = [];
    for (let h = 5; h <= 22; h++) {
    ...
    .then(data => {
        ...
        // Her saat dilimi için tablo satırı oluştur
        timeSlots.forEach(time => {
            ...
            hourTd.textContent = time.slice(0, 5); // HH:MM formatında göster
            ...
            days.forEach(day => {
                ...
                // Sadece saat ve dakikaya göre eşleştiriyoruz
                const match = data.find(entry =>
                    entry.day_of_week === day &&
                    entry.start_time.slice(0,5) === time.slice(0,5)
                );

                if (match) {
                    console.log(`🎯 Eşleşme bulundu -> Gün: ${day}, Saat: ${time}, Ders: ${match.title}`);
                    const a = document.createElement("a");
                    a.href = `/akademikx/public/teacher-panel/course-detail/${match.course_id}`;
